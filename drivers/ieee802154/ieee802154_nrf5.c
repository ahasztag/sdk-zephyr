...
static void nrf5_schedule_rx(uint8_t channel, uint32_t start, uint32_t duration)
{
	 ...
	 * the proper CSL Phase in the transmitted CSL Information Elements can be injected.
     *
	 * Note that even if this function is not called before the next CSL window starts
	 * (so no new delayed reception window is scheduled) the CSL phase will still be calculated
	 * as if the next windows were at times anchor_time + n * csl_period
	 */
	nrf_802154_csl_writer_anchor_time_set(nrf5_data.csl_rx_time);
}
